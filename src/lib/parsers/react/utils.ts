...
export function stripBlockBraces(code: string): string {
  ...
  if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
    ...
    // Dedent: find the minimum indentation among non-empty lines
...
/**
 * 从 useEffect 的 BlockStatement 中提取回调函数体
 * 排除 return 清理函数语句，只保留业务逻辑
 */
export function extractEffectBody(
  body: BabelTypes.BlockStatement,
  returnStmt: BabelTypes.ReturnStatement | undefined
): string {
  // Generate all statements except the cleanup return
  const statements = body.body.filter(s => s !== returnStmt);
  if (statements.length === 0) {
    return '';
  }
  const codeParts = statements.map(s => generate(s).code);
  const fullCode = codeParts.join('\n');
  return stripBlockBraces(fullCode.startsWith('{') ? fullCode : `{ ${fullCode} }`);
}

/**
 * 获取 AST 节点的源码文本（简化版本）
 * 通过遍历 AST 节点重构出大概的源码表示
 */
...
/**
 * 从 AST 节点中提取所有引用的标识符名称
 * 用于检测变量声明是否依赖其他状态变量
 */
export function extractReferencedIdentifiers(node: BabelTypes.Node): string[] {
  const identifiers: string[] = [];

  function walk(n: BabelTypes.Node): void {
    switch (n.type) {
      case 'Identifier':
        identifiers.push((n as BabelTypes.Identifier).name);
        break;
      case 'MemberExpression': {
        const mem = n as BabelTypes.MemberExpression;
        walk(mem.object);
        // Don't walk property for non-computed access (e.g., count.length -> just "count")
        if (mem.computed) walk(mem.property as BabelTypes.Expression);
        break;
      }
      case 'BinaryExpression': {
        const bin = n as BabelTypes.BinaryExpression;
        walk(bin.left as BabelTypes.Node);
        walk(bin.right as BabelTypes.Node);
        break;
      }
      case 'LogicalExpression': {
        const log = n as BabelTypes.LogicalExpression;
        walk(log.left as BabelTypes.Node);
        walk(log.right as BabelTypes.Node);
        break;
      }
      case 'UnaryExpression': {
        const unary = n as BabelTypes.UnaryExpression;
        walk(unary.argument);
        break;
      }
      case 'ConditionalExpression': {
        const cond = n as BabelTypes.ConditionalExpression;
        walk(cond.test as BabelTypes.Node);
        walk(cond.consequent as BabelTypes.Node);
        walk(cond.alternate as BabelTypes.Node);
        break;
      }
      case 'CallExpression': {
        const call = n as BabelTypes.CallExpression;
        walk(call.callee);
        for (const arg of call.arguments) {
          if (arg.type !== 'SpreadElement') walk(arg as BabelTypes.Expression);
        }
        break;
      }
      case 'TemplateLiteral': {
        const tmpl = n as BabelTypes.TemplateLiteral;
        for (const expr of tmpl.expressions) {
          walk(expr as BabelTypes.Expression);
        }
        break;
      }
      case 'ArrowFunctionExpression':
      case 'FunctionExpression':
        // Don't walk into function bodies - they define their own scope
        break;
      default:
        break;
    }
 ...
 */
export function reactEventToNative(reactEventName: string): string {
...
/**
 * 导出 @babel/generator 的兼容版本供其他模块使用
 */
export { generate };
